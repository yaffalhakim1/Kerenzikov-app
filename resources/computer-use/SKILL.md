...
Kerenzikov exposes the native Computer Use runtime directly to QuickJS, but initializes `sky` lazily. Do not import `@oai/sky` or call the raw Computer Use helper.
...
* It is usually not necessary to pause between performing an action and getting the updated app state. Kerenzikov automatically waits up to about one second after a recent action before capturing the new state.

## Reading screenshots

Screenshot URLs are in `screenshot.url`, and Kerenzikov returns them as base64-encoded `data:` URLs. To read a screenshot:
...
## Kerenzikov target restrictions

Waku Computer Use cannot control Kerenzikov itself, Codex, ChatGPT, Sky, password managers, System Settings, login or security prompts, Keychain Access, or terminal apps. Do not try to bypass these restrictions.
